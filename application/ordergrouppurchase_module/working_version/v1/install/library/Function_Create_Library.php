 ...
 */
Route::{$dataTypeL}(
    ':v/ordergrouppurchase_module/{$name}_route',
    'ordergrouppurchase_module/:v.controller.{$Name}Controller/{$name}{$DataTypeL}'
);
";
        // 判断文件是否存在
        if(file_exists("../route/ordergrouppurchase_route_v1_api.php")){
            file_put_contents(
                '../route/ordergrouppurchase_route_v1_api.php',
                $String, FILE_APPEND
            );
        }
    }
