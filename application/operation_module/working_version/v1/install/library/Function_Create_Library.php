 ...
 */
Route::{$dataTypeL}(
    ':v/operation_module/{$name}_route',
    'operation_module/:v.controller.{$Name}Controller/{$name}{$DataTypeL}'
);
";
        // 判断文件是否存在
        if(file_exists("../route/operation_route_v1_api.php")){
            file_put_contents(
                '../route/operation_route_v1_api.php',
                $String, FILE_APPEND
            );
        }
    }
