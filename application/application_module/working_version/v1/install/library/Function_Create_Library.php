 ...
 */
Route::{$dataTypeL}(
    ':v/application_module/{$name}_route',
    'application_module/:v.controller.{$Name}Controller/{$name}{$DataTypeL}'
);
";
        // 判断文件是否存在
        if(file_exists("../route/application_route_v1_api.php")){
            file_put_contents(
                '../route/application_route_v1_api.php',
                $String, FILE_APPEND
            );
        }
    }
