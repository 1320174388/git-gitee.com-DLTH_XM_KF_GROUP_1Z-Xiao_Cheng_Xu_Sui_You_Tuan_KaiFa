 ...
 */
Route::{$dataTypeL}(
    ':v/orderinter_module/{$name}_route',
    'orderinter_module/:v.controller.{$Name}Controller/{$name}{$DataTypeL}'
);
";
        // 判断文件是否存在
        if(file_exists("../route/orderinter_route_v1_api.php")){
            file_put_contents(
                '../route/orderinter_route_v1_api.php',
                $String, FILE_APPEND
            );
        }
    }
