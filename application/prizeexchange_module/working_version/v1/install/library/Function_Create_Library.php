 ...
 */
Route::{$dataTypeL}(
    ':v/prizeexchange_module/{$name}_route',
    'prizeexchange_module/:v.controller.{$Name}Controller/{$name}{$DataTypeL}'
);
";
        // 判断文件是否存在
        if(file_exists("../route/prizeexchange_route_v1_api.php")){
            file_put_contents(
                '../route/prizeexchange_route_v1_api.php',
                $String, FILE_APPEND
            );
        }
    }
