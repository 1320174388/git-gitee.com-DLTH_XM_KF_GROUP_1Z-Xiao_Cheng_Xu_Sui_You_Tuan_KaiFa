 ...
 */
Route::{$dataTypeL}(
    ':v/deductions_module/{$name}_route',
    'deductions_module/:v.controller.{$Name}Controller/{$name}{$DataTypeL}'
);
";
        // 判断文件是否存在
        if(file_exists("../route/deductions_route_v1_api.php")){
            file_put_contents(
                '../route/deductions_route_v1_api.php',
                $String, FILE_APPEND
            );
        }
    }
