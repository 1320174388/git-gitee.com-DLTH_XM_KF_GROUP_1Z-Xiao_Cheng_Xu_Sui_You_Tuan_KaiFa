 ...
 */
Route::{$dataTypeL}(
    ':v/ticketgroup_module/{$name}_route',
    'ticketgroup_module/:v.controller.{$Name}Controller/{$name}{$DataTypeL}'
);
";
        // 判断文件是否存在
        if(file_exists("../route/ticketgroup_route_v1_api.php")){
            file_put_contents(
                '../route/ticketgroup_route_v1_api.php',
                $String, FILE_APPEND
            );
        }
    }
