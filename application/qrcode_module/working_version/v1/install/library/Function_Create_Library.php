 ...
 */
Route::{$dataTypeL}(
    ':v/qrcode_module/{$name}_route',
    'qrcode_module/:v.controller.{$Name}Controller/{$name}{$DataTypeL}'
);
";
        // 判断文件是否存在
        if(file_exists("../route/qrcode_route_v1_api.php")){
            file_put_contents(
                '../route/qrcode_route_v1_api.php',
                $String, FILE_APPEND
            );
        }
    }
