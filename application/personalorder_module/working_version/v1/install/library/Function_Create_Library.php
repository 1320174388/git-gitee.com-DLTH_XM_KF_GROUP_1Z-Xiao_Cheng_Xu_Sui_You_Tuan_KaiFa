 ...
 */
Route::{$dataTypeL}(
    ':v/personalorder_module/{$name}_route',
    'personalorder_module/:v.controller.{$Name}Controller/{$name}{$DataTypeL}'
);
";
        // 判断文件是否存在
        if(file_exists("../route/personalorder_route_v1_api.php")){
            file_put_contents(
                '../route/personalorder_route_v1_api.php',
                $String, FILE_APPEND
            );
        }
    }
