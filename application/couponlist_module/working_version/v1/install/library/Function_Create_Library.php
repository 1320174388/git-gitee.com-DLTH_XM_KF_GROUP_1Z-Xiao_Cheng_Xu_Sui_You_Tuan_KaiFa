 ...
 */
Route::{$dataTypeL}(
    ':v/couponlist_module/{$name}_route',
    'couponlist_module/:v.controller.{$Name}Controller/{$name}{$DataTypeL}'
);
";
        // 判断文件是否存在
        if(file_exists("../route/couponlist_route_v1_api.php")){
            file_put_contents(
                '../route/couponlist_route_v1_api.php',
                $String, FILE_APPEND
            );
        }
    }
