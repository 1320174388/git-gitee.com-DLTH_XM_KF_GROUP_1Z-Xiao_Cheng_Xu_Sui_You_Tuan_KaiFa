 ...
 */
Route::{$dataTypeL}(
    ':v/grouppurchaselist_module/{$name}_route',
    'grouppurchaselist_module/:v.controller.{$Name}Controller/{$name}{$DataTypeL}'
);
";
        // 判断文件是否存在
        if(file_exists("../route/grouppurchaselist_route_v1_api.php")){
            file_put_contents(
                '../route/grouppurchaselist_route_v1_api.php',
                $String, FILE_APPEND
            );
        }
    }
