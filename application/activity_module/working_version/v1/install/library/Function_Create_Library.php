 ...
 */
Route::{$dataTypeL}(
    ':v/activity_module/{$name}_route',
    'activity_module/:v.controller.{$Name}Controller/{$name}{$DataTypeL}'
);
";
        // 判断文件是否存在
        if(file_exists("../route/activity_route_v1_api.php")){
            file_put_contents(
                '../route/activity_route_v1_api.php',
                $String, FILE_APPEND
            );
        }
    }
