 ...
 */
Route::{$dataTypeL}(
    ':v/wxloginadd_module/{$name}_route',
    'wxloginadd_module/:v.controller.{$Name}Controller/{$name}{$DataTypeL}'
);
";
        // 判断文件是否存在
        if(file_exists("../route/wxloginadd_route_v1_api.php")){
            file_put_contents(
                '../route/wxloginadd_route_v1_api.php',
                $String, FILE_APPEND
            );
        }
    }
